...
| Security  | Secure Enclave                   | We use the device hardware to generate the Private/Public key pair. The Private key _cannot_ leave the device.                                                                 |
...
| Fake Approval  | If coerced by an attacker, they use the Left Index finger. The device signs the challenge with a _different_ private key.                                                                             |
