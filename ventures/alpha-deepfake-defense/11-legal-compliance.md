...
|---------|----------------|
| **FIDO2 / WebAuthn Standard** | Critical. We **do not store face or fingerprint data**. The FaceID authentication happens locally on the iPhone's Secure Enclave. The iPhone signs a cryptographic payload with a private key. We only store the *Public Key*. 
