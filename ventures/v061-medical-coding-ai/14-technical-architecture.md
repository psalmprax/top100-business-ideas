...
| De-Identification | AWS Comprehend Medical                  | Scrubs Patient Names, SSNs, and Dates of Birth _before_ the text hits the core LLM processing queue.              |
...
| Zero Training Loop | We explicitly guarantee that Client A's patient charts will _never_ be used to fine-tune the master model.                                                                         |
