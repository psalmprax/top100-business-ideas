...
export const useCompliance = () => {
  ...
  useEffect(() => {
    ...
    setSimulationListener(endpoint => {
      toast.warning(`RECOVERY-FIRST: Compliance Service "${endpoint}" reported a drop. Activating simulation.`, {
  ...
  useEffect(() => {
    ...
    const fetchModelDetails = async () => {
      try {
        ...
        setModelAudits(scans.map((s: any) => ({
  ...
  const handleUpdateChecklistItem = async (id: string, assessment: any) => {
    try {
      const updated = await extendedApi.compliance.updateChecklistItem(id, assessment);
      setChecklists(prev => prev.map(item => item.id === id ? updated : item));
  ...
  const avgScore = totalModels > 0 
    ? Math.round(models.reduce((sum, m) => sum + (m.complianceScore || 0), 0) / totalModels) 
    : 0;
  ...
  const handleExportReport = async (modelId: string) => {
    ...
    try {
      ...
      toast.success("Report Generated", { description: `Exported as ${result.filename}` });
    } catch (e) {
  ...
  const handleEURegister = async (modelId?: string) => {
    try {
      const response = await extendedApi.compliance.eURegister(modelId || "global-cluster");
  ...
  const handleToggleGuardrail = async (key: string, value: boolean) => {
    ...
    try {
      ...
      await extendedApi.compliance.updateGuardrails(selectedModelForView.id, updatedGuardrails);
  ...
  return {
    state: {
      isAuthenticated, selectedModelForView, showModelDialog, activeTab, activeCategory,
      trainingModules, edgeDeployments, shadowAIDetections, complianceConnections,
      selectedAuditConnection, documentation, euDatabaseRegistered, registrationId,
      complianceBudget, proxyEndpoint, auditSearch, auditFilterType, reportType,
      roiMetrics, velocityTrends, deadlines, enterpriseAudits, modelBreakdown,
      modelAudits, modelHandshakes, modelArtifacts, regionalReports, financialMetrics,
      articles, loadingArticles, models, audits, incidents, vendors, isLoading,
      showVendorDialog, showUploadDialog, showEdgeLogDialog, selectedEdgeDevice,
    ...
    actions: {
      setSelectedModelForView, setShowModelDialog, setActiveTab, setActiveCategory,
