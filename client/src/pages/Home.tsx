...
import { exportToCSV, exportToPDF, exportShortlistToPDF } from "@/lib/exportUtils";

const HERO_BG = "https://d2xsxph8kpxj0f.cloudfront.net/310519663416294596/Z5RySCNbJcUs3jbtGjPeMT/hero-bg-ad6ZRYLG9KdpGvzmjGjnUz.webp";
  ...
  return (
    <button
      ...
      style={{
        ...
        border: isBookmarked ? "1px solid oklch(0.22 0.18 145 / 0.5)" : "1px solid oklch(0.22 0.02 265)",
      }}
...
function StatCounter({ value, label, prefix = "", suffix = "" }: { value: number; label: string; prefix?: string; suffix?: string }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <div className="text-4xl font-bold font-display text-white mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs font-semibold text-amber-400">{pct >= 50 ? "Very High" : pct >= 25 ? "High" : "Medium"}</span>
      </div>
      <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
        <div
          className="h-full rounded-full transition-all duration-700"
...
function IdeaCard({ idea, index, onClick, onBookmark, isBookmarked, onCompare, isInComparison }: { idea: BusinessIdea; index: number; onClick: () => void; onBookmark: () => void; isBookmarked: boolean; onCompare: () => void; isInComparison: boolean }) {
  ...
  return (
    ...
    >
      {/* Rank badge */}
      <div
        className="absolute top-4 right-4 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold font-display"
      ...
      <h3 className="font-bold text-white mb-2 pr-10 leading-tight" style={{ fontFamily: "'Syne', sans-serif", fontSize: "0.95rem" }}>
        {idea.title}
      </h3>

      <p className="text-white/50 text-xs mb-4 line-clamp-2 leading-relaxed">{idea.description}</p>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {idea.markets.slice(0, 3).map(m => (
            <span key={m} className="text-xs px-1.5 py-0.5 rounded bg-white/8 text-white/40">{m}</span>
          ))}
          {idea.markets.length > 3 && <span className="text-xs px-1.5 py-0.5 rounded bg-white/8 text-white/40">+{idea.markets.length - 3}</span>}
        </div>
      </div>

      {/* Profit margin indicator */}
      <div className="mt-3 pt-3 border-t border-white/8 flex items-center justify-between">
        <span className="text-xs text-white/40">Profit Margin</span>
        <span className="text-xs font-bold text-emerald-400">{idea.profit_margin}%</span>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button
          ...
          style={{
            ...
            border: isInComparison ? "1px solid oklch(0.65 0.2 250 / 0.5)" : "1px solid oklch(0.22 0.02 265)",
          }}
        >
...
function IdeaModal({ idea, onClose }: { idea: BusinessIdea; onClose: () => void }) {
  const catColor = CATEGORY_COLORS[idea.category] || "#6366f1";
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" style={{ background: "oklch(0.10 0.02 265)", border: "1px solid oklch(0.25 0.04 265)" }}>
        <DialogHeader>
          <div className="flex items-start gap-3 mb-2">
            ...
            <div>
              <span className="text-xs font-semibold px-2 py-0.5 rounded-md mb-1 inline-block" style={{ backgroundColor: catColor + "18", color: catColor }}>
                {idea.category}
              </span>
              <DialogTitle className="text-white text-xl leading-tight" style={{ fontFamily: "'Syne', sans-serif" }}>
                {idea.title}
              </DialogTitle>
            </div>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="p-4 rounded-xl border" style={{ background: "oklch(0.12 0.025 265)", borderColor: trendColor + "33" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold text-white" style={{ fontFamily: "'Syne', sans-serif" }}>Gap Analysis</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { icon: <DollarSign className="w-4 h-4" />, label: "Earning Potential", value: idea.earning_label, color: "#f59e0b" },
              { icon: <Clock className="w-4 h-4" />, label: "Rollout Speed", value: idea.rollout_label, color: "#22c55e" },
              { icon: <TrendingUp className="w-4 h-4" />, label: "Profit Margin", value: `${idea.profit_margin}%`, color: "#3b82f6" },
              { icon: <BarChart2 className="w-4 h-4" />, label: "Market Size", value: `$${idea.market_size_bn}B`, color: "#a78bfa" },
              { icon: <Zap className="w-4 h-4" />, label: "Startup Cost", value: idea.startup_cost, color: "#fb923c" },
              { icon: <Activity className="w-4 h-4" />, label: "Trend", value: idea.trend, color: trendColor },
          ...
          {/* Radar Chart */}
          <div>
            <h4 className="text-sm font-semibold text-white/70 mb-3" style={{ fontFamily: "'Syne', sans-serif" }}>Opportunity Score</h4>
          ...
          <div className="flex flex-wrap gap-2">
            {idea.markets.map(m => (
              <span key={m} className="flex items-center gap-1 text-xs px-2 py-1 rounded-full" style={{ background: "oklch(0.18 0.03 250)", color: "#60a5fa" }}>
                <Globe className="w-3 h-3" />{m}
              </span>
            ))}
            {idea.tags.map(t => (
              <span key={t} className="text-xs px-2 py-1 rounded-full text-white/50" style={{ background: "oklch(0.16 0.02 265)" }}>
                #{t}
              </span>
            ))}
          </div>
...
function ChartsSection({ ideas }: { ideas: BusinessIdea[] }) {
  ...
  const catData = useMemo(() => {
    ...
    ideas.forEach(i => { counts[i.category] = (counts[i.category] || 0) + 1; });
    return Object.entries(counts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([name, value]) => ({ name: name.replace(" & ", " & ").replace("Technology", "Tech"), value, color: CATEGORY_COLORS[name] || "#6366f1" }));
  }, [ideas]);

  // Trend distribution
  const trendData = useMemo(() => {
    const counts: Record<string, number> = {};
    ideas.forEach(i => { counts[i.trend] = (counts[i.trend] || 0) + 1; });
    return Object.entries(counts).map(([name, value]) => ({ name, value, color: TREND_COLORS[name] }));
  }, [ideas]);

  // Top 10 by earning potential
  const top10Earning = useMemo(() =>
    [...ideas].sort((a, b) => b.earning_potential - a.earning_potential).slice(0, 10)
      .map(i => ({ name: i.title.split(":")[0].substring(0, 22) + (i.title.length > 22 ? "…" : ""), value: i.earning_potential / 1000000, color: CATEGORY_COLORS[i.category] || "#6366f1" })),
    [ideas]
  );

  // Startup cost vs profit margin scatter
  const scatterData = useMemo(() => {
    const costMap: Record<string, number> = { "Very Low ($1K–$5K)": 3, "Very Low ($2K–$10K)": 6, "Very Low ($2K–$15K)": 8, "Low ($5K–$20K)": 12, "Low ($5K–$25K)": 15, "Low ($5K–$30K)": 17, "Low ($10K–$40K)": 25, "Low ($10K–$50K)": 30, "Low ($10K–$60K)": 35, "Low ($15K–$60K)": 37, "Low ($15K–$70K)": 42, "Low ($15K–$80K)": 47, "Medium ($20K–$100K)": 60, "Medium ($25K–$120K)": 72, "Medium ($30K–$150K)": 90, "Medium ($40K–$200K)": 120, "Medium ($50K–$250K)": 150, "High ($100K–$400K)": 250, "High ($100K–$500K)": 300, "High ($200K–$1M)": 600 };
    return ideas.map(i => ({ x: costMap[i.startup_cost] || 50, y: i.profit_margin, z: i.rollout_speed, name: i.title, color: CATEGORY_COLORS[i.category] || "#6366f1" }));
  }, [ideas]);

  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload?.length) {
      return (
        <div className="rounded-lg p-3 text-xs" style={{ background: "oklch(0.15 0.03 265)", border: "1px solid oklch(0.25 0.04 265)" }}>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
      {/* Category Bar Chart */}
      <div className="rounded-2xl p-6" style={{ background: "oklch(0.12 0.02 265)", border: "1px solid oklch(0.22 0.02 265)" }}>
        <h3 className="text-white font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "'Syne', sans-serif" }}>
          <BarChart2 className="w-4 h-4 text-blue-400" /> Ideas by Category (Top 10)
        </h3>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={catData} layout="vertical" margin={{ left: 0, right: 20 }}>
            ...
            <YAxis type="category" dataKey="name" tick={{ fill: "rgba(255,255,255,0.6)", fontSize: 10 }} width={120} axisLine={false} tickLine={false} />
      ...
      <div className="rounded-2xl p-6" style={{ background: "oklch(0.12 0.02 265)", border: "1px solid oklch(0.22 0.02 265)" }}>
        <h3 className="text-white font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "'Syne', sans-serif" }}>
          <PieChartIcon className="w-4 h-4 text-emerald-400" /> Growth Trend Distribution
        </h3>
        <ResponsiveContainer width="100%" height={260}>
          <PieChart>
            <Pie data={trendData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={4} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
      ...
      <div className="rounded-2xl p-6" style={{ background: "oklch(0.12 0.02 265)", border: "1px solid oklch(0.22 0.02 265)" }}>
        <h3 className="text-white font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "'Syne', sans-serif" }}>
          <DollarSign className="w-4 h-4 text-amber-400" /> Top 10 by Earning Potential ($M/yr)
        </h3>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={top10Earning} layout="vertical" margin={{ left: 0, right: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} />
            <XAxis type="number" tick={{ fill: "rgba(255,255,255,0.4)", fontSize: 10 }} axisLine={false} tickLine={false} unit="M" />
            <YAxis type="category" dataKey="name" tick={{ fill: "rgba(255,255,255,0.6)", fontSize: 10 }} width={130} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} />
            <Bar dataKey="value" radius={[0, 4, 4, 0]} name="Max Earning ($M)">
              {top10Earning.map((entry, i) => <Cell key={i} fill={entry.color} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="rounded-2xl p-6" style={{ background: "oklch(0.12 0.02 265)", border: "1px solid oklch(0.22 0.02 265)" }}>
        <h3 className="text-white font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "'Syne', sans-serif" }}>
          <Activity className="w-4 h-4 text-violet-400" /> Startup Cost vs. Profit Margin
        </h3>
        <p className="text-white/40 text-xs mb-3">Bubble size = rollout speed. Top-left = best opportunities.</p>
        <ResponsiveContainer width="100%" height={220}>
          <ScatterChart margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
            <XAxis dataKey="x" name="Startup Cost ($K)" tick={{ fill: "rgba(255,255,255,0.4)", fontSize: 10 }} axisLine={false} tickLine={false} label={{ value: "Cost ($K)", fill: "rgba(255,255,255,0.3)", fontSize: 10, position: "insideBottom", offset: -5 }} />
            <YAxis dataKey="y" name="Profit Margin %" tick={{ fill: "rgba(255,255,255,0.4)", fontSize: 10 }} axisLine={false} tickLine={false} label={{ value: "Margin %", fill: "rgba(255,255,255,0.3)", fontSize: 10, angle: -90, position: "insideLeft" }} />
            ...
            <Tooltip content={({ active, payload }) => {
              if (active && payload?.length) {
                ...
                return (
                  <div className="rounded-lg p-3 text-xs" style={{ background: "oklch(0.15 0.03 265)", border: "1px solid oklch(0.25 0.04 265)" }}>
                    <p className="text-white font-semibold mb-1 max-w-40">{d?.name}</p>
            ...
            <Scatter data={scatterData} shape={(props: any) => {
              ...
              return <circle cx={cx} cy={cy} r={props.r || 5} fill={payload.color} fillOpacity={0.7} stroke={payload.color} strokeWidth={1} />;
            }} />
          </ScatterChart>
        </ResponsiveContainer>
      </div>
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedMarket !== "all") result = result.filter(i => i.markets.includes(selectedMarket));
    if (selectedTrend !== "all") result = result.filter(i => i.trend === selectedTrend);

    return [...result].sort((a, b) => {
      if (sortBy === "rank") return a.rank - b.rank;
      if (sortBy === "earning") return b.earning_potential - a.earning_potential;
  ...
  const hasFilters = search || selectedCategory !== "all" || selectedMarket !== "all" || selectedTrend !== "all";
  ...
  const isInComparison = (ideaId: number) => comparisonIdeas.some(i => i.id === ideaId);
  ...
  // Summary stats
  const explosiveCount = businessIdeas.filter(i => i.trend === "Explosive").length;
  const avgMargin = Math.round(businessIdeas.reduce((s, i) => s + i.profit_margin, 0) / businessIdeas.length);
  ...
  return (
    <div className="min-h-screen" style={{ background: "oklch(0.09 0.02 265)" }}>
      {/* NAV */}
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b" style={{ background: "oklch(0.09 0.02 265 / 0.9)", borderColor: "oklch(0.22 0.02 265)" }}>
              ...
              <button
                onClick={() => {
                  if (shortlistedIdeas.length === 0) {
                    toast.info('No ideas in shortlist yet. Click the bookmark icon on any idea to add it.');
                  } else {
                    toast.info(`${shortlistedIdeas.length} ideas in your shortlist. Use the export menu to download.`);
                  }
                }}
                className="relative p-2 rounded-lg transition-all hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="relative overflow-hidden" style={{ minHeight: "480px" }}>
        ...
        />
        <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, oklch(0.09 0.02 265 / 0.3), oklch(0.09 0.02 265))" }} />

        <div className="relative container pt-20 pb-16">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-6" style={{ background: "oklch(0.65 0.2 250 / 0.15)", color: "#93c5fd", border: "1px solid oklch(0.65 0.2 250 / 0.3)" }}>
              <Zap className="w-3 h-3" /> Gap Analysis · High-Earning Opportunities · Fast Rollout
            </div>
      ...
      <div className="container pb-20">
        ...
        {showCharts && <ChartsSection ideas={filtered.length > 0 ? filtered : businessIdeas} />}

        {/* Filter Bar */}
        <div className="rounded-2xl p-5 mb-8" style={{ background: "oklch(0.12 0.02 265)", border: "1px solid oklch(0.22 0.02 265)" }}>
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <Select value={selectedMarket} onValueChange={v => { setSelectedMarket(v); setVisibleCount(24); }}>
              ...
              <SelectContent style={{ background: "oklch(0.14 0.02 265)", border: "1px solid oklch(0.25 0.04 265)" }}>
                ...
                {ALL_MARKETS.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Select value={selectedTrend} onValueChange={v => { setSelectedTrend(v); setVisibleCount(24); }}>
              ...
              <SelectContent style={{ background: "oklch(0.14 0.02 265)", border: "1px solid oklch(0.25 0.04 265)" }}>
                ...
                {ALL_TRENDS.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
          ...
          <div className="flex items-center gap-3 mt-3 pt-3 border-t border-white/8">
            ...
            {hasFilters && (
              <div className="flex flex-wrap gap-1.5">
                {selectedCategory !== "all" && <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300">{selectedCategory}</span>}
                {selectedMarket !== "all" && <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300">{selectedMarket}</span>}
                {selectedTrend !== "all" && <span className="text-xs px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300">{selectedTrend}</span>}
              </div>
            )}
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-white/40 text-lg">No ideas match your filters.</p>
            <button onClick={clearFilters} className="mt-4 text-blue-400 hover:text-blue-300 text-sm underline">Clear all filters</button>
          </div>
        ) : (
          <>
            ...
            {visibleCount < filtered.length && (
              <div className="text-center">
                <button
                  ...
                  style={{ background: "linear-gradient(135deg, #3b82f6, #22c55e)", color: "white", fontFamily: "'Syne', sans-serif" }}
                >
      ...
      {/* Modal */}
      {selectedIdea && (selectedIdea.gtm_strategy ? (
        <IdeaDetailEnhanced idea={selectedIdea} onClose={() => setSelectedIdea(null)} />
      ) : (
        <IdeaModal idea={selectedIdea} onClose={() => setSelectedIdea(null)} />
      ))}

      {/* Comparison View */}
      {showComparison && <ComparisonView ideas={comparisonIdeas} onClose={() => setShowComparison(false)} />}
    </div>
  );
}
