...
import { CATEGORY_COLORS, TREND_COLORS, type BusinessIdea } from "@/lib/api";
...
export default function Home() {
  const {
    ...
    setVisibleCount,
    comparisonIdeas,
    setComparisonIdeas: _setComparisonIdeas,
    showShortlistOnly: _showShortlistOnly,
    setShowShortlistOnly: _setShowShortlistOnly,
    isLoading,
    businessIdeas,
    filtered,
    visibleIdeas,
    shortlistedIdeas: _shortlistedIdeas,
    hasFilters,
    clearFilters,
    addToComparison,
    isInComparison,
  ...
  return (
    <div
      className={`min-h-screen ${layoutMode === "omega" ? "bg-white" : ""}`}
      style={{
        background: layoutMode === "omega" ? "#ffffff" : "oklch(0.09 0.02 265)",
      }}
    >
      <div className="container mx-auto px-4">{renderLayout()}</div>
      ...
      {comparisonIdeas.length > 0 && (
        ...
        >
          <Activity className="w-4 h-4" /> Compare Ventues (
          {comparisonIdeas.length}/3)
        </button>
      )}
    </div>
  );
...
function IdeaModal({ idea, onClose }: { idea: BusinessIdea; onClose: () => void }) {
  ...
  const _trendColor = TREND_COLORS[idea.trend];
...
function MetricBox({ icon: Icon, label, value, color }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string | number; color: string }) {
  return (
