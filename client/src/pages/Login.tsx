...
import { Eye, EyeOff, Loader2, Mail, Lock, ArrowRight, Rocket, ShieldCheck, Cpu, Fingerprint, Zap, Bot, Users } from 'lucide-react';

const PRODUCT_MAPPING: Record<string, { label: string; icon: any; color: string; path: string }> = {
    'agent-ops': { label: 'AgentOps', icon: Cpu, color: 'text-blue-400', path: '/products/agent-ops' },
    'ai-compliance': { label: 'Compliance', icon: ShieldCheck, color: 'text-green-400', path: '/products/ai-compliance' },
    'deepfake-defense': { label: 'Deepfake Defense', icon: Fingerprint, color: 'text-purple-400', path: '/products/deepfake-defense' },
    'denial-defense': { label: 'Denial Defense', icon: ShieldCheck, color: 'text-red-400', path: '/products/denial-defense' },
    'actionable-ai': { label: 'Actionable AI', icon: Zap, color: 'text-yellow-400', path: '/products/actionable-ai' },
    'workflow-bot': { label: 'Workflow Bot', icon: Bot, color: 'text-cyan-400', path: '/products/workflow-bot' },
    'alpha-workforce': { label: 'Workforce', icon: Users, color: 'text-indigo-400', path: '/products/workforce' },
};
...
export default function Login() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            window.location.href = PRODUCT_MAPPING[productKey]?.path || '/products/agent-ops';
        } catch (err: any) {
                    ...
                    {!requiresSelection ? (
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                ...
                                <TabsContent value="login">
                                    <form onSubmit={handleLogin} className="space-y-4">
                                            ...
                                            {selectedProduct && (
                                                <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        {(() => {
                                                            const P = PRODUCT_MAPPING[selectedProduct] || PRODUCT_MAPPING['agent-ops'];
                                                            const Icon = P.icon;
                                                            return (
                                                                <>
                                                                    <div className={`p-2 bg-slate-800 rounded-lg ${P.color}`}>
                                                                        <Icon className="w-4 h-4" />
                                                                    </div>
                                                                    <div>
                                                                        <div className="text-xs text-slate-400 font-medium uppercase tracking-wider">Signing into</div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <Link href="/forgot-password" title="Forgot password?">
                                                        <span className="text-sm text-blue-400 hover:text-blue-300 cursor-pointer">Forgot password?</span>
                                                    </Link>
                                                </div>
                                                <div className="relative">
                                        ...
                                        <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 font-bold" disabled={isLoading} data-testid="btn-signin">
                                            {isLoading ? (
                                                <>
                                ...
                                <TabsContent value="signup">
                                    <form onSubmit={handleSignUp} className="space-y-4">
                                        ...
                                        <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 font-bold" disabled={isLoading} data-testid="btn-signup">
                                            {isLoading ? (
                                                <>
                                                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                                                    Creating account...
                        ...
                        <div className="space-y-6">
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-sm text-slate-400">Please choose which product you'd like to sign into</p>
                            </div>
                            
                            <div className="grid gap-3">
                                {availableProducts.map(productId => {
                                    const P = PRODUCT_MAPPING[productId] || { label: productId, icon: Zap, color: 'text-slate-400', path: '/products/agent-ops' };
                                    const Icon = P.icon;
                                    return (
                                        <button
                                            key={productId}
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="text-xs text-slate-500 uppercase tracking-widest mt-0.5">Active Subscription</div>
                                            </div>
