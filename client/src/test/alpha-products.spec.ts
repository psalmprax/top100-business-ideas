...
test.describe('AlphaAI Products', () => {
    ...
    test.describe('Landing Page', () => {
        ...
        test('should show all 3 products', async ({ page }) => {
            ...
            await expect(page.getByText('AI Compliance Hub', { exact: true })).toBeVisible();
            await expect(page.getByText('Deepfake Defense', { exact: true })).toBeVisible();
        });

        test('should navigate to product pages', async ({ page }) => {
            await page.goto('/');
            await page.locator('[data-testid="venture-card"]').filter({ hasText: 'AgentOps' }).getByRole('button', { name: 'Learn More' }).click();
    ...
    test.describe('Billing Page', () => {
        ...
        test('should show payment method section', async ({ page }) => {
            ...
            await expect(page.locator('[data-slot="card-title"]').getByText('Payment Method', { exact: true })).toBeVisible();
        ...
        test('should show invoice history', async ({ page }) => {
            ...
            await expect(page.locator('[data-slot="card-title"]').getByText('Invoice History', { exact: true })).toBeVisible();
    ...
    test.describe('Dashboard Product Pages', () => {
        ...
        test('should load Deepfake Defense Force Dashboard', async ({ page }) => {
            ...
            await expect(page.getByText('LivenessLink', { exact: false }).first()).toBeVisible();
        });
    });

    test.describe('Secondary Alpha Products', () => {
        test('should load DenialDefense and show recovery rate', async ({ page }) => {
        ...
        test('should load FreelancerWorkflowBot and show authorization', async ({ page }) => {
