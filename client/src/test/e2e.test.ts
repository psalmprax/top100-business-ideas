...
describe("Navigation E2E Tests", () => {
  ...
  test("should navigate to venture detail", async () => {
    ...
    await page.waitForSelector('[data-testid="venture-card"]', {
      timeout: VENTURE_TIMEOUT,
    });
...
describe("Alpha Agent Ops - E2E Tests", () => {
  ...
  test("should display agent metrics", async () => {
    ...
    await expect(
      page.locator('[data-testid="metric-total-requests"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="metric-total-cost"]')
    ).toBeVisible();
...
describe("Alpha AI Act Compliance - E2E Tests", () => {
  ...
  test("should run compliance check", async () => {
    ...
    // Wait for results
    await page.waitForSelector('[data-testid="check-results"]', {
      timeout: VENTURE_TIMEOUT,
    });
...
describe("Alpha Deepfake Defense - E2E Tests", () => {
  ...
  test("should upload and analyze media", async () => {
    const page = await createPage();
    await page.goto(
      `${BASE_URL}/ventures/alpha-hecta-deepfake-defense/analyze`
    );
    ...
    await expect(
      page.locator('[data-testid="result-confidence"]')
    ).toBeVisible();
