...
export function NewAgentDialog({ open, onOpenChange, onSave }: ModalProps) {
  ...
  const getModelOptions = (provider: string) => {
    switch (provider) {
      case "openai": return ["gpt-4o", "gpt-4-turbo", "o1-preview", "gpt-3.5-turbo"];
      case "anthropic": return ["claude-3-5-sonnet", "claude-3-opus", "claude-3-haiku"];
      case "google": return ["gemini-1.5-pro", "gemini-1.5-flash"];
      case "mistral": return ["mistral-large", "mistral-medium", "codestral"];
      case "meta": return ["llama-3.1-405b", "llama-3.1-70b"];
      case "gross": return ["gross-v1-optimized"];
      ...
      <DialogContent className="sm:max-w-[550px] bg-slate-900 text-white border-slate-700">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Provision a new autonomous intelligence unit into the platform fabric.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-6 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Select value={data.environment} onValueChange={v => setData({...data, environment: v})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Select value={data.tier} onValueChange={v => setData({...data, tier: v})}>
                <SelectTrigger className="bg-slate-800 border-slate-700">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
                  <SelectItem value="strategic">Strategic (High-Level Plan)</SelectItem>
                  <SelectItem value="tactical">Tactical (Mission Ops)</SelectItem>
                  <SelectItem value="industrial">Industrial (High-Throughput)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <Select value={data.provider} onValueChange={v => setData({...data, provider: v, model: getModelOptions(v)[0]})}>
                <SelectTrigger className="bg-slate-800 border-slate-700">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
            ...
            <div className="grid gap-2">
              ...
              <Select value={data.model} onValueChange={v => setData({...data, model: v})}>
                <SelectTrigger className="bg-slate-800 border-slate-700">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <Switch 
                  ...
                  onCheckedChange={v => setData({...data, persistent_memory: v})}
                />
              </div>
              <p className="text-[10px] text-slate-500">
                Enable recursive context storage for long-term mission coherence.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            <div className="space-y-3">
              <Label className="flex items-center justify-between">
                ...
                <span className="text-blue-400 font-mono text-xs">{data.temperature}</span>
              </Label>
              <Slider 
                min={0} 
                max={1.5} 
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Configure an external callback for platform events.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {["AGENT_ERROR", "BUDGET_EXCEEDED", "SECURITY_ALERT", "SYSTEM_HEALED"].map(ev => (
                <div key={ev} className="flex items-center space-x-2">
                  <Checkbox 
                    id={ev} 
                    checked={data.events.includes(ev)}
                    onCheckedChange={(checked) => {
                      if (checked) setData({...data, events: [...data.events, ev]});
                      else setData({...data, events: data.events.filter(e => e !== ev)});
                    }}
                  />
                  <label htmlFor={ev} className="text-xs">{ev}</label>
                </div>
              ))}
            </div>
          </div>
