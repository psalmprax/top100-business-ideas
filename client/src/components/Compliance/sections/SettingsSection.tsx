    ...
    try {
      ...
      toast.success(
        result?.status === "healthy"
          ? "Proxy verified and connected"
          : "Proxy verification failed"
      );
    } catch (err) {
      setGatewayStatus("inactive");
      toast.error("Failed to verify proxy endpoint");
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Global controls for EU AI Act enforcement and regional adaptations
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="sso" className="w-full">
        ...
        <TabsContent value="sso">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure enterprise identity providers for Article 15 access
                control.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/20">
                  <div className="space-y-0.5">
                    <Label className="text-sm">
                      Auto-Provision Compliance Officers
                    </Label>
                    <p className="text-xs text-muted-foreground">
        ...
        <TabsContent value="budget">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enforce financial guardrails to prevent non-compliant resource
                spend.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
        ...
        <TabsContent value="network">
          <Card>
            <CardHeader>
              <CardTitle className="text-base font-bold">
                Regulated Proxy Endpoint
              </CardTitle>
              <CardDescription>
                Direct all LLM traffic through the Sentinel Compliance Gateway.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                ...
                <div
                  className={`p-4 ${gatewayStatus === "active" ? "bg-emerald-500/5 border-emerald-500/20" : "bg-red-500/5 border-red-500/20"} rounded-lg flex gap-3`}
                >
                  ...
                  <div>
                    <div
                      className={`text-sm font-bold ${gatewayStatus === "active" ? "text-emerald-700" : "text-red-700"}`}
                    >
                      {gatewayStatus === "active"
                        ? "Gateway Active"
                    ...
                    <p
                      className={`text-xs ${gatewayStatus === "active" ? "text-emerald-600/80" : "text-red-600/80"}`}
                    >
                      {gatewayStatus === "active"
                        ? "99.9% of traffic is currently being scrubbed for PII and Bias."
                        : "Unable to connect to compliance proxy. Check endpoint URL."}
                    </p>
                  </div>
                </div>
              </div>
              <Button onClick={handleVerifyProxy} disabled={saving}>
        ...
        <TabsContent value="alerts">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Receive immediate alerts for critical Article 71/72 events.
              </CardDescription>
            </CardHeader>
