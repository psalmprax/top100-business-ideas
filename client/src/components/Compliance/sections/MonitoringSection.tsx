  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-20">
        ...
        <p className="mt-4 text-muted-foreground text-sm font-medium">Attaching to live compliance stream...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Real-time telemetry from automated EU AI Act guardrails</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {(metrics?.events || []).length > 0 ? (
              metrics.events.map((event: any, i: number) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg border bg-muted/30">
                  <div className={`p-1.5 rounded-md ${event.severity === 'high' ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-500'}`}>
                    {event.severity === 'high' ? <AlertTriangle className="w-4 h-4" /> : <Zap className="w-4 h-4" />}
                  </div>
                  <div className="flex-1">
                    <div className="text-xs font-bold">{event.title}</div>
                    <div className="text-[10px] text-muted-foreground">{event.description}</div>
                  </div>
                  <div className="text-[9px] text-muted-foreground font-mono">{new Date(event.timestamp).toLocaleTimeString()}</div>
                </div>
              ))
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                <Activity className="w-10 h-10 opacity-20 mb-2" />
                <p className="text-xs">No active compliance deviations detected.</p>
              </div>
            )}
          </div>
        </CardContent>
