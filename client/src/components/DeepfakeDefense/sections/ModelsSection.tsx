      ...
      <CardHeader className="flex flex-row items-center justify-between border-b border-border/50 bg-purple-500/5 py-6">
        <div>
          ...
          <CardDescription className="text-sm font-medium opacity-70">
            Enterprise-specific neural networks and specialized weights for targeted defense.
          </CardDescription>
        </div>
        <Button
      ...
      <CardContent className="p-0">
        <Table>
          <TableHeader className="bg-muted/30">
            <TableRow>
              <TableHead className="text-[10px] font-black uppercase tracking-widest pl-6">Model Signature</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-widest">Neural Network</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-widest">Efficiency</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-widest">Status</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-widest text-right pr-6">Metadata</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {customModels.map((model) => (
              <TableRow key={model.id} className="hover:bg-purple-500/5 transition-colors border-b border-border/30 last:border-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">
                        Version {model.version} &middot; {model.type || 'Foresnsic'}
                      </div>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="space-y-1.5 min-w-[120px]">
                    <div className="flex justify-between text-[10px] font-bold">
                      ...
                      <span className="text-purple-400">{(model.accuracy * 100).toFixed(1)}%</span>
                    </div>
                    <Progress
