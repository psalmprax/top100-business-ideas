        ...
        <CardContent className="p-0">
          <div className="aspect-video bg-zinc-950/50 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden group">
            ...
            <div className="absolute top-4 left-4 flex gap-2 z-20">
              <Badge variant="secondary" className="bg-black/60 backdrop-blur-md border-white/10 text-[9px] text-white/50 px-2 py-0.5">
            ...
            <div className="relative z-10 flex flex-col items-center">
              {isAnalyzing ? (
                ...
                <div className="space-y-6 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-black text-white">Biometric Enclave Ready</h3>
                    <p className="text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
                      Authorize camera access to initiate <span className="text-white font-bold">rPPG pulse heart-rate validation</span> and phoneme synchronization.
                    </p>
                  </div>
                  <Button
                    size="lg"
                    ...
                    onClick={async () => {
                      const tId = toast.loading("Initializing Secure Camera Protocol...");
                      try {
                        ...
                        try {
                          const result = await extendedApi.deepfake.analyzeEnterprise({
                            source: "live_camera",
                          ...
                          setAdvancedResult({
                            ...
                            confidence: result?.forensic_score || result?.confidence || 0.994,
                            liveness_verified: result?.liveness_verified ?? true,
                            bpm: result?.bpm || 72,
                          ...
                          toast.success("Human Presence Authenticated", {
                            description: "Forensic report injected into audit trail."
                          });
                          handleDownload("liveness-report.pdf", "ENTERPRISE_LIVENESS_CERTIFICATE");
                        } catch (e) {
                          if (videoTrack) videoTrack.stop();
                          clearInterval(timer);
                          setIsAnalyzing(false);
                        ...
                        toast.error("Handshake Failed", { id: tId, description: "Camera access denied or interface busy." });
                      }
                    }}
                  >
                    Authenticate Now
      ...
      <div className="space-y-6">
        <Card className="border-border/50 bg-card/50">
          <CardHeader>
            ...
            <CardDescription className="text-[10px] uppercase font-bold tracking-widest opacity-50">Local Biometric Audit</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
              ...
              {sessions.map((session) => (
                <div key={session.id} className="p-4 hover:bg-muted/10 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-mono text-[9px] opacity-40 uppercase tracking-tighter">ID: {session.id}</span>
                    <Badge
                      variant={session.status === "verified" ? "outline" : "destructive"}
                      className={`text-[8px] h-3.5 leading-none px-1.5 font-black uppercase ${
                        session.status === 'verified' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : ''
                      }`}
                    >
                      {session.status}
                    </Badge>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex flex-col">
                      <span className="text-[11px] font-black uppercase tracking-tight">{session.type}</span>
                      <span className="text-[9px] text-muted-foreground">Pulse/Phoneme Verification</span>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-black italic">{session.microExpressionScore || session.voiceLivenessScore || "0"}%</div>
                      <div className="text-[8px] uppercase tracking-widest opacity-40 font-bold">Match</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <Card className="border-emerald-500/30 bg-emerald-500/5 animate-in slide-in-from-right-4 duration-500">
            <CardContent className="pt-6 space-y-4">
               <div className="flex items-center gap-3">
                 ...
                 <div className="text-sm font-black text-emerald-400">Identity Authenticated</div>
               </div>
               <div className="grid grid-cols-2 gap-2 text-[10px]">
                  <div className="p-2 rounded bg-emerald-500/10 border border-emerald-500/20">
                     <div className="opacity-50 font-bold uppercase mb-1">Heart Rate</div>
                     <div className="text-lg font-black">{advancedResult.bpm} BPM</div>
                  </div>
                  <div className="p-2 rounded bg-emerald-500/10 border border-emerald-500/20">
                     <div className="opacity-50 font-bold uppercase mb-1">Confidence</div>
                     <div className="text-lg font-black">{(advancedResult.confidence * 100).toFixed(2)}%</div>
                  </div>
               </div>
            </CardContent>
          </Card>
