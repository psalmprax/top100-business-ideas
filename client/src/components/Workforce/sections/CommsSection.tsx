      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="bg-gradient-to-br from-indigo-500/5 to-purple-600/5 border-primary/10 shadow-xl backdrop-blur-md">
          <CardHeader className="py-6 border-b border-white/5">
            <CardTitle className="text-xl font-black flex items-center gap-3">
              <Settings2 className="w-6 h-6 text-indigo-400" /> Webhook
              Orchestration
            </CardTitle>
            <CardDescription className="text-xs font-medium opacity-60">
              Link autonomous agent discourse to corporate settlement &
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const action = slackIntegration
                        ? extendedApi.workforce.updateIntegration(
                            slackIntegration.id,
                            { url: webhooks.slack }
                          )
                        : extendedApi.workforce.createIntegration({
                            type: "slack",
                            name: "Slack Bridge",
                            url: webhooks.slack,
                            enabled: true,
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="h-12 w-12 rounded-xl bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-all"
                    onClick={() => {
                      const tgIntegration = integrations.find(
                        i => i.type === "telegram"
                      );
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="h-12 w-12 rounded-xl bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-all"
                    onClick={() => {
                      const discordIntegration = integrations.find(
                        i => i.type === "discord"
                      );
                      if (!webhooks.discord) {
        ...
        <Card className="border-border/50 bg-card/30 backdrop-blur-md shadow-2xl overflow-hidden">
          ...
          <CardContent className="p-8 space-y-4">
            {[
              {
                label: "#boardroom-discourse",
                active: !!webhooks.slack,
                platform: "Slack",
              },
              {
                label: "AlphaExecutiveBot",
                active: !!webhooks.telegram,
                platform: "Telegram",
              },
              {
                label: "Agent-Alerts-Alpha",
                active: !!webhooks.discord,
                platform: "Discord",
              },
            ].map((ch, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-5 rounded-3xl border border-white/5 bg-background/50 shadow-inner group hover:border-white/10 transition-all"
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-3 h-3 rounded-full ${ch.active ? "bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)] animate-pulse" : "bg-muted-foreground/20"}`}
                  />
                  <div>
            ...
            <div className="pt-6 mt-2 text-center">
              <p className="text-[10px] text-muted-foreground/40 italic leading-relaxed">
                "Internal mesh utilizes{" "}
                <span className="text-indigo-400 font-bold">AES-256-GCM</span>{" "}
                for inter-agent communication nodes."
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <ScrollArea className="flex-grow">
            <div className="p-4 space-y-2">
              {agents.map((agent: any) => (
                ...
                >
                  <div
                    className={`p-2 rounded-xl ${selectedRecipient === agent.id ? "bg-white/20" : "bg-indigo-500/10"} transition-all`}
                  >
                    {agent.id === "all" ? (
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`text-xs font-black tracking-tight ${selectedRecipient === agent.id ? "text-white" : "text-foreground"}`}
                    >
                      {agent.name}
                    </span>
                    <span
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 mt-1">
                  {selectedRecipient === "all"
                    ? "Protocol: Public Discourse"
                    : `Direct Tunnel ID: ${selectedRecipient.toUpperCase()}`}
                </p>
              </div>
            </div>
              ...
              {filteredMessages.map((msg: any) => (
                ...
                >
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/30 mt-3 px-2">
                    {msg.sender === "user" ? "Transmitted" : "Received"}{" "}
                    &middot;{" "}
              ...
              <div className="flex gap-4">
                <div className="flex-grow relative group">
                  ...
                  <div className="absolute right-5 top-1/2 -translate-y-1/2">
                    <div
                      className={`w-2 h-2 rounded-full ${isSending ? "bg-amber-400 animate-pulse" : "bg-emerald-500"}`}
                    />
                  </div>
                </div>
                <Button
                ...
                {[
                  {
                    ...
                    prompt:
                      "What is the workforce consensus on our current trajectory?",
                  },
                  {
                    icon: Zap,
                    label: "Strategic Audit",
                    prompt:
                      "Audit our current outreach vectors for performance leaks.",
                  },
                  {
                    icon: Brain,
                    label: "Reasoning Deep-Dive",
                    prompt:
                      "Reason together: How can we reduce our ecosystem CPA by 30%?",
                  },
                ].map((s, i) => (
                  <button
                    key={i}
