...
export function useAgentOps() {
  ...
  const [complianceStatus, setComplianceStatus] = useState({ hipaa: "PENDING", sox: "PENDING", gdpr: "PENDING" });
  ...
  const [architectureDefaults, setArchitectureDefaults] = useState({ temperature: 0.7, maxTokens: 4000, budget: 10 });
  const [newAgentData, setNewAgentData] = useState<any>({
    name: "", type: "langgraph", environment: "production", provider: "openai", model: "gpt-4o",
    budget: 10, maxTokens: 100000, org_id: "", control_webhook: "", metadata: {},
    tier: "industrial", persistent_memory: true
  });
    ...
    try {
      ...
      const transformedAgents: DashboardAgent[] = (Array.isArray(agentsRes) ? agentsRes : []).map(agent => ({
        ...
        config: {
          ...
          maxTokens: agent.config?.maxTokens || architectureDefaults.maxTokens,
          temperature: agent.config?.temperature || architectureDefaults.temperature,
      ...
      if (isAuthenticated) {
        extendedApi.sso.config("default").then(conf => { if (conf) setSsoConfig(conf); }).catch(() => {});
      }
    } catch (error) {
  ...
  const toggleAgentStatus = async (agentId: string) => {
    ...
    try {
      if (newStatus === "active") await extendedApi.agents.start(agentId);
  ...
  const handleDecommissionAgent = async (agentId: string) => {
    ...
    if (!window.confirm(`PERMANENTLY DECOMMISSION NODE ${agent.name} (${agent.id})? This action cannot be undone and will purge all transient state.`)) {
      return;
    }

    try {
      await extendedApi.agents.delete(agentId);
      toast.success(`Agent ${agent.name} decommissioned and purged from cluster.`);
      refreshData();
    } catch (error) {
  ...
  const handleUpdateAssets = async (agent: DashboardAgent) => {
    try {
      await extendedApi.agents.update(agent.id, { 
        metadata: { ...agent.metadata, assets_sync_initiated: new Date().toISOString() } 
      } as any);
  ...
  return {
    ...
    agents, auditLog, activeTab, setActiveTab, activeCategory, setActiveCategory,
    isLoading, auditSearchQuery, setAuditSearchQuery, auditFilterOutcome, setAuditFilterOutcome,
    budgetRules, webhooks, multiCloudStatus, selfHealingEvents, clusterNodes,
    llmConfigs, alertConfigs, vigilanceAlerts, roiMetrics, healingConfigs,
    usageForecasts, ssoConfig, complianceStatus, liveMetrics,
    researchTopic, setResearchTopic, isResearching, researchResult,
    strategyPrompt, setStrategyPrompt, isGeneratingStrategy, strategyResult,
    showSettingsDialog, setShowSettingsDialog, selectedAgent, setSelectedAgent,
    showNewAgentDialog, setShowNewAgentDialog, newAgentData, setNewAgentData,
    refreshData, toggleAgentStatus, handleUpdateAgent, handleCreateAgent,
    handlePaperclipResearch, handleHermesStrategy, toggleAlert, handleSelfHealingToggle,
    handleTriggerPanic, handleSystemReset, handleDecommissionAgent,
    handleRunComplianceAudit, handleInjectHint, handleUpdateAssets, handleConfigureStream,
