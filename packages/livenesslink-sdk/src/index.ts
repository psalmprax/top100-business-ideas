...
export class LivenessLinkClient {
    ...
    constructor(config: LivenessConfig) {
        ...
        this.client.interceptors.response.use(
            response => response,
            error => {
                console.error('[LivenessLink] API Error:', error.response?.data || error.message);
    ...
    async submitLivenessProof(
        ...
        const response = await this.client.post(`/liveness/sessions/${sessionId}/verify`, formData, {
    ...
    async createVerificationSession(userId: string): Promise<VerificationSession> {
        const response = await this.client.post('/verification/sessions', { userId });
        return response.data;
    }
    ...
    async submitVerification(
        ...
        const response = await this.client.post(`/verification/sessions/${sessionId}/verify`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
        });
        return response.data;
    }
    ...
    async getVerificationResult(sessionId: string): Promise<VerificationResult> {
        const response = await this.client.get(`/verification/sessions/${sessionId}`);
        return response.data;
    }
    ...
    async enrollBiometric(
        ...
        const response = await this.client.post(`/biometrics/${userId}/enroll`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
     ...
     */
    async deleteBiometricTemplate(userId: string, templateId: string): Promise<void> {
    ...
    async getFraudAlerts(options?: {
        ...
        const response = await this.client.get('/fraud/alerts', { params: options });
        return response.data;
    }
...
export function createLivenessLinkClient(config: LivenessConfig): LivenessLinkClient {
