...
class DuressDetectionService:
    ...
    def check_duress_pin(
        ...
        # In production, would check against registered duress PINs
        # Simulated: any PIN ending in 00 triggers duress
        
        is_duress_pin = pin.endswith("00")
    ...
    def _trigger_duress_alert(
        ...
        logger.warning(
            f"DURESS ALERT: User {session.user_id} - Level: {level.value}"
        )
    ...
    def complete_session(self, session_id: str) -> Dict[str, Any]:
        ...
        if not session.alerts:
            ...
            session.verdict = {
                "is_duress": is_duress,
                "confidence": 1.0 - avg_probability if not is_duress else avg_probability,
