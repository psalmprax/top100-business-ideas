...
class ReportingService:
    ...
    def generate_annex_iv_report(self, check: ComplianceCheck) -> Dict[str, Any]:
        ...
        status_str = str(check.status.value) if hasattr(check.status, 'value') else str(check.status)
        ...
        return {
            ...
            "sections": [
                ...
                },
                {
                    ...
                    "findings": [f.get("description", "") for f in findings if f.get("severity") == "high"],
                    "status": "Evidence collected via GitHub Connector"
                },
                {
                    "id": "3",
                    "title": "Information about Monitoring and Control",
                    "findings": [f.get("description", "") for f in findings if f.get("rule", "").startswith("Article 15")],
