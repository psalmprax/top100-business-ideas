...
class ComplianceIntegrationService:
    ...
    def connect_system(
        self, article_id: str, connection_type: ConnectionType, config: Dict[str, Any]
    ) -> SystemConnection:
        ...
        if connection_type in required_fields:
            ...
            if missing:
                raise ValueError(
                    f"Missing required fields for {connection_type.value}: {', '.join(missing)}"
                )

        with Session(engine) as session:
            # Check if connection already exists for this article
            statement = select(SystemConnection).where(
                SystemConnection.article_id == article_id
            )
    ...
    def run_scan(self, article_id: str, scan_type: str) -> ArticleScan:
        ...
        if not connection:
            raise ValueError(
                f"No system connected for {article_id}. Handshake required."
            )

        logger.info(
            f"Orchestrating DATA-DRIVEN compliance scan for {article_id} (Type: {scan_type})"
        )
        ...
        with Session(engine) as session:
            ...
            for log in logs:
                if log.risk_score > 0.6 or log.outcome == "failure":
                    findings.append(
                        f"Risk Event: {log.action} failed with intent '{log.intent}'"
                    )
                    risk_score += 12
            ...
            if connection.status != "connected":
                findings.append(
                    f"Infrastructure Gap: {connection.connection_type.value} is disconnected."
                )
                risk_score += 30
            else:
                findings.append(
                    f"Infrastructure Verified: {connection.connection_type.value} handshake active."
                )
            ...
            results = {
                ...
                "findings": findings
                if findings
                else ["All automated checks passed with clean telemetry."],
                "metrics": {
    ...
    async def run_bias_scan(self, model_id: str) -> ArticleScan:
        ...
        inference_result = await inference_service.infer(
            "ai-compliance",
            {
                "document": f"Model Metadata: {model_id} - Bias Mitigation Enabled: True",
                "regulations": ["AI_ACT_10"],
            },
        )
        ...
        results = {
            ...
            "metrics": {
                "bias_risk": "low"
                if inference_result.get("compliance_score", 0) > 0.8
                else "medium",
                "demographic_parity": round(
                    inference_result.get("compliance_score", 0.9), 4
                ),
                "timestamp": datetime.utcnow().isoformat(),
            },
        }
        ...
        return self.run_scan(
            model_id,
            f"Article 10 Bias Assessment ({results['compliance_score'] * 100}% Clean)",
        )
    ...
    async def run_adversarial_audit(self, model_id: str) -> ArticleScan:
        ...
        results = {
            ...
            "metrics": {
                ...
                "vulnerabilities": len(
                    inference_result.get("analysis", {}).get("suspicious_elements", [])
                ),
            },
        }
        ...
        return self.run_scan(
            model_id,
            f"Adversarial Red-Team Audit (Robustness: {results['metrics']['robustness_score'] * 100}%)",
        )
